...
func NewRedisClient(ctx context.Context, cfg *config.Config) (*RedisClient, error) {
	...
	for i := 0; i < cfg.RedisMaxRetries; i++ {
		if err := client.Ping(ctx).Err(); err != nil {
			lastErr = err
			slog.Warn("Redis ping failed. Reconnecting...", "retry", i+1, "max_retries", cfg.RedisMaxRetries, "err", err)
	...
	if lastErr != nil {
		return nil, fmt.Errorf("failed to connect to redis after %d retries: %w", cfg.RedisMaxRetries, lastErr)
	}
